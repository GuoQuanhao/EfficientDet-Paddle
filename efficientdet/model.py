...
class Regressor(nn.Layer):
    ...
    def __init__(self, in_channels, num_anchors, num_layers, pyramid_levels=5, onnx_export=False):
        ...
        self.bn_list = nn.LayerList(
            [nn.LayerList([nn.BatchNorm2D(in_channels, momentum=0.99, epsilon=1e-3) for i in range(num_layers)]) for j in
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.reshape([feat.shape[0], -1, 4])
...
class Classifier(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        for feat, bn_list in zip(inputs, self.bn_list):
            ...
            feat = feat.transpose([0, 2, 3, 1])
            feat = feat.reshape([feat.shape[0], feat.shape[1], feat.shape[2], self.num_anchors,
                                          self.num_classes])
            feat = feat.reshape([feat.shape[0], -1, self.num_classes])
